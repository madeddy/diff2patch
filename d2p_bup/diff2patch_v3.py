...
class D2p_Common:
    ...
    def cc(cls):
        max_cc = cpu_count()
        cls.use_cc = int(max_cc * 0.75 if max_cc > 6 else max_cc * 0.5
                         if max_cc > 2 else 1)
...
class DirTreeCmp(D2p_Common, filecmp.dircmp):
    ...
    def __init__(self, old, new, ignore=None, hide=None, shallow=True):
        ...
        self.left = self.check_inpath(old)
        self.right = self.check_inpath(new)
        self.ignore.extend([
    ...
    def diff_survey(self):
        ...
        # control print
        # print(
        #     f"NEW: {self.new_only_all} DIFF: {self.diff_all} FUNNY: {self.funny_all}")
    ...
    def run_compare(self):
        ...
        # NOTE: Not needed: stared unpacking does it also
        # self.diff_survey()
        self.survey_lst = [*self.new_only_all, *self.diff_all, *self.funny_all]
        # control print
        print(f"SURVEY: {self.new_only_all}")
...
class D2p(D2p_Common):
    ...
    def __init__(self, survey_lst, new_pt, out_base_pt=None):
        ...
        self.inp_pt = self.check_inpath(new_pt)
